...
SERVER_NAME=$(assign_default_if_empty $3 'qtactivityscriptsrvnosql1')
...
echo "az cosmosdb create -n ${SERVER_NAME} -g ${RESOURCE_GROUP} --locations ${LOCATION}"
exit 0
if [[ $SERVER_EXISTS == "1" ]]; then
    echo "Server with ${SERVER_NAME} already exists"
else
    # Create a server
    az cosmosdb create -n ${SERVER_NAME} -g ${RESOURCE_GROUP} --locations regionName=${LOCATION}
fi
